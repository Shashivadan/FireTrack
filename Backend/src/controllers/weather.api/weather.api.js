...
export async function automaticPrediction(req, res) {
  try {
    ...
    if (!lat || !lon) return res.status(403).json({ message: "places provide lat and  lon" });
    ...
    if (!data) return res.status(403).json({ message: "server is can't be reached" })
    ...
    if (!response.data) return res.status(403).json({ message: "model server is down" })
